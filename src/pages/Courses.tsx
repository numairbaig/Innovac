...
export default function Courses() {
  return (
    <>
      <SEO title="Courses | INNOVAC BIOTECHNOLOGIES" />
        ...
        <div className="max-w-[1400px] mx-auto relative z-20 w-full">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-12 mt-4 lg:mt-0 w-full">
            <div className="max-w-3xl lg:w-3/5">
              ...
              >
                <Breadcrumb items={[{ label: 'Courses' }]} accentColor="text-[#FF4D00]" />
                <PageLabel accentColor="text-[#FF4D00]">SCIENTIFIC TRAINING</PageLabel>
                <h1 className="text-5xl md:text-7xl lg:text-[76px] font-medium tracking-tight leading-[1.05] mb-8 text-white">
                  Scientific<br />
                  <span className="text-[#FF4D00]">Courses.</span>
                </h1>
                <p className="text-lg text-neutral-300 max-w-2xl mb-16 leading-relaxed font-light">
                  Comprehensive educational courses in biotechnology and bioinformatics. Gain hands-on practical skills and deep theoretical knowledge.
                </p>
              </motion.div>
            </div>
            
            {/* Right Visual Area */}
            <PageHeroIllustration page="courses" className="lg:w-2/5" />
          </div>
        </div>
      </section>

      <section className="py-24 px-6 bg-white text-center">
        <h2 className="text-3xl font-medium mb-6">Advance Your Scientific Skills</h2>
        <p className="text-neutral-600 mb-12 max-w-2xl mx-auto">
          Our courses provide in-depth training on various scientific methodologies, from molecular biology techniques to advanced computational biology tools. Connect with us to learn about upcoming course schedules.
        </p>
        <div className="flex flex-wrap items-center justify-center gap-4">
          <Button href="/contact?type=workshop" variant="primary" withArrow>REQUEST COURSE DETAILS</Button>
          <Button href="/workshops" variant="outline" withArrow>EXPLORE WORKSHOPS</Button>
        </div>
      </section>
    </>
  );
