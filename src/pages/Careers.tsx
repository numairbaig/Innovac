...
export default function Careers() {
  return (
    <>
      <SEO title="Careers | INNOVAC BIOTECHNOLOGIES" />
        ...
        <div className="max-w-[1400px] mx-auto relative z-20 w-full">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-12 mt-4 lg:mt-0 w-full">
            <div className="max-w-3xl lg:w-3/5">
              ...
              >
                <Breadcrumb items={[{ label: 'Careers' }]} accentColor="text-[#FF4D00]" />
                <PageLabel accentColor="text-[#FF4D00]">JOIN OUR TEAM</PageLabel>
                <h1 className="text-5xl md:text-7xl lg:text-[76px] font-medium tracking-tight leading-[1.05] mb-8 text-white">
                  Join Our Scientific<br />
                  <span className="text-[#FF4D00]">Mission.</span>
                </h1>
                <p className="text-lg text-neutral-300 max-w-2xl mb-16 leading-relaxed font-light">
                  Explore career opportunities at INNOVAC BIOTECHNOLOGIES. We are looking for passionate individuals to push the boundaries of biotechnology.
                </p>
              </motion.div>
            </div>
            
            {/* Right Visual Area */}
            <PageHeroIllustration page="careers" className="lg:w-2/5" />
          </div>
        </div>
      </section>

      <section className="py-24 px-6 bg-white text-center">
        <h2 className="text-3xl font-medium mb-6">Current Openings</h2>
        <p className="text-neutral-600 mb-12 max-w-2xl mx-auto">
          We are always looking for talented scientists, researchers, and professionals to join our growing team. While we do not have specific openings listed at this moment, we welcome general applications.
        </p>
        <Button href="/contact?type=general" variant="primary" withArrow>SEND YOUR CV</Button>
      </section>
    </>
  );
}